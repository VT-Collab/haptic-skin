...
for method in ["none", "GUI", "local", "global"]:
    ...
    for user_n in range(1, n+1):
        ...
        # time users spend correcting?
        # instead of mean_error write a function that searches through the data points and
        # measures how much time passed if there was a significant change in the feature

        def compute_time(var, t, start_marg, end_marg):
            # print(var.shape)
            # print(len(time))
            # print(start_marg)
            # print(end_marg)
            arr = np.array([0,2,3,6,7,0])
            print(arr)
            print(np.diff(arr))
            idx = np.argwhere(np.abs(np.diff(arr)) > 2.0) + 1
            print(idx)
            exit()

        if method == "GUI":
            # z["user_" + str(user_n)] = compute_time(points[2,:][:marg_1_idx])
            z["user_" + str(user_n)] = compute_time(points[2,:], time, None, marg_1_idx)
